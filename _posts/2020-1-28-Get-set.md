...
summary: |-
      <h1>Quick research and possible reference guide if I'm lazy (Taken from my MCSA book)</h1><p>My tidbit on setting a DNS address for a server in windows server; There are two ways of configuring an Ethernet Device for DNS; <code>sconfig</code> (server config) or
